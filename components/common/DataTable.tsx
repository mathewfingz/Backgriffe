    ...
    <div className="space-y-3">
      ...
      <div className="overflow-auto rounded-xl border">
        <table className="min-w-full text-sm">
          <thead><tr>{columns.map(c=> <th key={c.key} className="px-3 py-2 text-left">{c.header}</th>)}</tr></thead>
          <tbody>{filtered.map(r=> (
            <tr key={r.id} className="border-t">
              {columns.map(c=> <td key={c.key} className="px-3 py-2">{c.render? c.render(r): String((r as any)[c.key])}</td>)}
            </tr>
          ))}</tbody>
        </table>
      </div>
